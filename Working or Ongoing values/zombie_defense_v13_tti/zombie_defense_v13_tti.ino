// ME350 Zombie Defense v13 - TTI-BASED SEQUENCING
// =================================================
// Targeting Mode: Time To Impact (TTI) prioritization
// Sequencing: Orders targets by when they'll hit (lowest TTI = most urgent)
// Best for: Predictable zombie speeds, good velocity estimation
// 
// Key characteristics:
// - Calculates TTI = remaining_distance / velocity
// - Sorts sequence by effective TTI (accounts for travel time)
// - Emergency override at 85%+ and 95%+ thresholds
// - Falls back to distance when TTI unavailable
...
//============================================
// LANE PRIORITY (TTI mode adjustments)
//============================================
const float LANE_PRIORITY[4] = {1.0, 1.15, 1.15, 1.1};
...
const int SHORT_LANE_TTI_BOOST = 200;  // Subtract from L2/L3 TTI

// Distance thresholds (fallback)
const float EARLY_ENGAGE_THRESHOLD_LONG = 0.08;
const float EARLY_ENGAGE_THRESHOLD_SHORT = 0.05;
const float MIN_ENGAGE_THRESHOLD = 0.95;

// Override thresholds
const float OVERRIDE_THRESHOLD[4] = {0.90, 0.50, 0.50, 0.90};
const float ABSOLUTE_EMERGENCY_THRESHOLD = 0.85;
const float EXTREME_EMERGENCY_THRESHOLD = 0.95;
...
float calculateThreatScore_TTI(int lane);
float getEffectiveTTI(int lane);
bool canReachInTime(int lane);
void calculateNewSequence_TTI();
...
void setup() {
  ...
  Serial.println(F("MODE: TTI-BASED SEQUENCING"));
...
//============================================
// GET EFFECTIVE TTI
// Accounts for travel time and lane priority
//============================================
float getEffectiveTTI(int lane) {
  ...
  // Short lane boost - make L2/L3 appear more urgent
...
//============================================
// UPDATE TTI
// TTI = remaining_distance / velocity
//============================================
void updateTTI() {
  for (int i = 0; i < 4; i++) {
...
//============================================
// CALCULATE SEQUENCE - TTI BASED
// Sorts by effective TTI (lowest first = will hit soonest)
//============================================
void calculateNewSequence_TTI() {
  ...
  for (int i = 0; i < 4; i++) {
    ...
    // L4 gets priority boost when past 70%
    if (i == 3 && zombieDistances[i] >= 0.70) {
      lanes[i].effectiveTTI -= 300;
    }

    if (lanes[i].effectiveTTI < 0) lanes[i].effectiveTTI = 0;
    if (lanes[i].tti >= 99999) lanes[i].effectiveTTI = 99999;
  ...
  // Sort by EFFECTIVE TTI (LOWEST first = closest to impact)
  for (int i = 0; i < 3; i++) {
    for (int j = i + 1; j < 4; j++) {
  ...
  for (int i = 0; i < sequenceCount; i++) {
    DBG_PRINT(F("L")); DBG_PRINT(targetSequence[i] + 1);
    DBG_PRINT(F("(")); DBG_PRINT((int)getEffectiveTTI(targetSequence[i])); DBG_PRINT(F(") "));
  }
  DBG_PRINTLN();
}

//============================================
// CALCULATE THREAT SCORE - TTI BASED
// Primary: TTI (lower = more urgent)
// Fallback: Distance when TTI unavailable
//============================================
float calculateThreatScore_TTI(int lane) {
  ...
  // TTI-BASED SCORING (lower TTI = higher score)
  if (rawTTI < 99999 && effectiveTTI < 5000) {
    if (effectiveTTI < 200) {
      score = 1200 + (200 - effectiveTTI) * 6;  // Extreme urgency
    ...
    // DISTANCE FALLBACK when TTI unavailable
    if (dist > 0.90) {
  ...
  // Short lane protection
  if (laneIsShort[lane]) {
    for (int other = 0; other < 4; other++) {
      if (other == lane) continue;
      if (laneIsLong[other] && ProxSensors[other].direction == FORWARD && !laneIsCritical[other]) {
        score += 2000;
        break;
      }
    }
...
bool shouldRecalculateForCriticalLane() {
  ...
  for (int i = 0; i < 4; i++) {
    ...
    // Urgent if TTI very low and lower than current
    if (tti < URGENT_TTI_THRESHOLD && tti < currentTTI - 200) {
      return true;
    }

    // Also trigger on distance as backup
    if (dist >= 0.70 && dist < MIN_ENGAGE_THRESHOLD) {
      return true;
    }
  }
  return false;
  ...
  for (int i = 0; i < 4; i++) {
    if (ProxSensors[i].direction == FORWARD) {
      ...
      if (zombieDistances[i] >= minThreshold && zombieDistances[i] < MIN_ENGAGE_THRESHOLD) {
        ...
        float tti = getEffectiveTTI(i);
        if (!recentlyAttempted && tti < bestTTI) {
          bestTTI = tti;
          bestLane = i;
        }
      }
...
void loop() {
  ...
  if (calibrationActive) {
    ...
    //============================================
    // NORMAL OVERRIDE CHECK (TTI-based)
    //============================================
    int overrideLane = -1;
    float bestOverrideTTI = 99999;
    ...
    if (canCheckOverride) {
      ...
      for (int i = 0; i < 4; i++) {
        ...
        // TTI-based override criteria
        if (dist > EXTREME_EMERGENCY_THRESHOLD && ProxSensors[i].direction == FORWARD) {
          shouldOverride = true;
        } else if (tti < currentTTI - 500 && tti < 1500) {
          shouldOverride = true;  // 500ms faster and under 1.5s = urgent
        ...
        if (shouldOverride && tti < bestOverrideTTI) {
          overrideLane = i;
          bestOverrideTTI = tti;
        }
      }
    ...
    if (overrideLane >= 0 && overrideLane != committedLane) {
      if (!(overrideLane == lastOverrideLane && timeSinceOverride < 5000)) {
        ...
        DBG_PRINT(F(" TTI=")); DBG_PRINT((int)getEffectiveTTI(overrideLane)); DBG_PRINTLN(F(" !!!"));
...
void runPIDController() {
  ...
  if (abs(error) > 5) voltage += (voltage > 0) ? FRICTION_LEFT : -FRICTION_RIGHT;
...
void setMotorVoltage(float voltage) {
  ...
  if (voltage > 0.01) { digitalWrite(MOTOR_IN2, LOW); digitalWrite(MOTOR_IN3, HIGH); }
  else if (voltage < -0.01) { digitalWrite(MOTOR_IN2, HIGH); digitalWrite(MOTOR_IN3, LOW); }
  else { digitalWrite(MOTOR_IN2, LOW); digitalWrite(MOTOR_IN3, LOW); }
...
void homeToLeftLimit() {
  ...
  systemEnabled = true;
  while (digitalRead(LIMIT_LEFT) == HIGH) { setMotorVoltage(1.5); delay(1); }
  stopMotor();
  encoder.write(0);
  desiredPosition = 0;
...
void processSerialCommands() {
  ...
  switch (cmd) {
    case 'T': case 't': case 'Y': case 'y':
      Serial.println(F("MODE: TTI-BASED"));
      resetTTITracking();
      resetSequence();
      startAutoMode();
      startCalibration();
...
void printHelp() {
  Serial.println(F("\n--- COMMANDS ---"));
  Serial.println(F("T/Y - Start (TTI mode)"));
...
void printCalibrationStatus() {
  ...
  for (int i = 0; i < 4; i++) {
    ...
    Serial.print(F("% TTI=")); Serial.println((int)timeToImpact[i]);
  }
}
...
void printStatus() {
  ...
  for (int i = 0; i < 4; i++) {
    ...
    Serial.print(F("(")); Serial.print((int)(timeToImpact[i])); Serial.print(F(")"));
  }
  if (committedLane >= 0) {
