...
// TIGHT thresholds - only TRUE emergencies trigger override
// L4 needs MUCH lower threshold (higher %) because travel time is longest
// L1: 96%, L2/L3: 94%, L4: 88% (earlier trigger for far lane)
const float OVERRIDE_THRESHOLD[4] = {0.04, 0.06, 0.06, 0.12};  // L4 triggers at 88%
const float ABSOLUTE_OVERRIDE_DISTANCE = 0.05;  // 95%+ absolute emergency
...
const unsigned long MIN_DWELL_TIME = 350;       // Increased from 250 - dwell longer at targets
...
const unsigned long NORMAL_DWELL_TIME = 600;   // Increased from 500
const unsigned long MAX_DWELL_TIME = 1500;     // Increased from 1200
const unsigned long L4_DWELL_TIME = 2000;      // Increased from 1800

const unsigned long BACKWARD_CONFIRM_TIME = 80;  // Increased from 40 - confirm retreat longer
...
bool shouldOverride(int newLane) {
  ...
  // RULE 1: If we're AT the target (dwelling), only override for EXTREME emergency (<2%)
  // This is 98%+ on display - almost certain miss otherwise
  if (atTarget && ProxSensors[committedLane].direction == FORWARD) {
    return newDist < 0.02;  // 98%+ on display - VERY strict
  }
  
  // RULE 2: If we're nearing target and committed lane still needs attention,
  // only override if new lane is MUCH more critical (3x closer)
  if (nearingTarget && committedDist < 0.40 && 
      ProxSensors[committedLane].direction == FORWARD) {
    return newDist < committedDist * 0.3;  // New must be 3x closer
  }
  ...
  if (newDist < threshold) {
    ...
    if (committedDist < 0.25 && ProxSensors[committedLane].direction == FORWARD) {
      return newDist < committedDist * 0.4;  // Stricter - 2.5x closer required
    }
    return true;
  }
...
void calculateNewBatch() {
  ...
  struct LaneInfo {
    int lane;
    float sortScore;    // Used for sorting - meaning depends on mode
    float distance;
    float velocity;
    bool isActive;
  };
  ...
  for (int i = 0; i < 4; i++) {
    ...
    lanes[i].velocity = zombieVelocities[i];
    ...
    if (lanes[i].isActive) {
      // Calculate sort score based on current MODE
      // Lower score = higher priority (will be sorted first)
      
      if (targetMode == MODE_DISTANCE) {
        // DISTANCE MODE: Sort purely by distance (closest first)
        // Lower distance = higher priority
        lanes[i].sortScore = lanes[i].distance;
        
      } else if (targetMode == MODE_TTI) {
        // TTI MODE: Sort purely by velocity (fastest approaching first)
        // NOTE: velocity is POSITIVE when zombie approaches (distance decreasing)
        float approachRate = lanes[i].velocity;  // Already positive for approaching zombies
        if (approachRate > 0.0001) {
          // Faster approach = lower TTI = higher priority
          lanes[i].sortScore = lanes[i].distance / approachRate;
        } else {
          // Not approaching or moving backward - low priority
          lanes[i].sortScore = 99999;
        }
        
      } else {
        // HYBRID MODE: Distance DOMINATES, velocity is minor tie-breaker
        // A zombie at 80% should ALWAYS beat one at 70%, regardless of velocity
        // Weight: 95% distance, 5% velocity (velocity only breaks ties)
        float distScore = lanes[i].distance;  // 0 to 1, lower = closer = more urgent
        
        // Velocity component: small bonus for fast-approaching zombies
        // NOTE: velocity is POSITIVE when zombie approaches (distance decreasing)
        float approachRate = lanes[i].velocity;
        float velBonus = 0.0f;
        if (approachRate > 0.0001) {
          // Small bonus for fast approach (max 0.05 reduction to score)
          velBonus = min(approachRate * 50.0f, 0.05f);
        }
        
        // LANE TRAVEL TIME ADJUSTMENT:
        // L2/L3 (middle lanes) get bonus - faster to reach
        // L4 (far lane) gets penalty - takes longest to reach, needs earlier attention
        // L1 neutral (closest, fast travel)
        float laneAdjust = 0.0f;
        if (i == 1 || i == 2) {  // L2 or L3 - shorter travel
          laneAdjust = -0.03f;  // Small boost (reduce score = higher priority)
        } else if (i == 3) {  // L4 - longest travel time
          laneAdjust = 0.05f;  // Penalty (increase score = must be closer to compete)
        }
        
        // Combined score: distance with small velocity and lane adjustments
        // Lower = higher priority
        lanes[i].sortScore = distScore - velBonus + laneAdjust;
      }
      ...
      if (i == lastHitLane && consecutiveSameLane >= MAX_CONSECUTIVE_SAME) {
        ...
        if (millis() - lastHitTime < 1500) {
          lanes[i].sortScore += 10.0;  // Large penalty to push to end
        }
      }
      // Smaller penalty for just being the last hit lane (encourage rotation)
      else if (i == lastHitLane && millis() - lastHitTime < 800) {
        lanes[i].sortScore += 0.5;  // Small penalty to encourage others first
      }
  ...
  static bool skipMessagePrinted = false;  // Only print skip message once
  if (activeLanes == 1 && consecutiveSameLane >= MAX_CONSECUTIVE_SAME) {
    ...
    if (activeLane == lastHitLane && millis() - lastHitTime < 1000) {
      batchActive = false;
      if (analysisMode && !skipMessagePrinted) {
        Serial.print(F("[BATCH] Skipping L"));
        Serial.print(activeLane + 1);
        Serial.println(F(" - cooldown"));
        skipMessagePrinted = true;  // Don't spam
      }
      return;
    }
  }
  // Reset skip message flag when we're not skipping
  skipMessagePrinted = false;
  
  // Sort lanes by sortScore (lowest first = most urgent)
  // Simple bubble sort for 4 elements
  for (int i = 0; i < 3; i++) {
    for (int j = i + 1; j < 4; j++) {
      if (lanes[j].sortScore < lanes[i].sortScore) {
  ...
  // Build batch: Add each active lane ONCE in priority order
  ...
  // ALWAYS output batch info (important for debugging sequencing)
  if (batchActive) {
    Serial.print(F("[SEQ] "));
    ...
    // DEBUG: Show sort scores for first batch calculation
    if (analysisMode) {
      Serial.print(F("[SCORE] "));
      for (int i = 0; i < batchSize; i++) {
        int lane = targetBatch[i];
        Serial.print(F("L"));
        Serial.print(lane + 1);
        Serial.print(F("@"));
        Serial.print((int)((1.0 - zombieDistances[lane]) * 100));
...
int getNextBatchTarget() {
  ...
  while (batchIndex < batchSize) {
    ...
    // Target no longer valid - log why and skip it
    if (analysisMode && lane >= 0 && lane < 4) {
      Serial.print(F("[SKIP] L"));
      Serial.print(lane + 1);
      if (ProxSensors[lane].direction != FORWARD) {
        Serial.println(F(" - moving back"));
      } else if (zombieDistances[lane] >= EARLY_ENGAGE_THRESHOLD) {
...
void loop() {
  ...
  for (int i = 0; i < 4; i++) {
    // Calculate raw normalized distance (0 = at wall, 1 = far away)
    float rawDist = (ProxSensors[i].currVal - ProxRange[i][1]) / 
                    (float)(ProxRange[i][0] - ProxRange[i][1]);
    
    // SHRINK effective range by 10% to account for momentum
    // This makes 95% on physical scale = 100% on display
    // And 5% on physical scale = 0% on display
    // Formula: effective = (raw - 0.05) / 0.90
    // This maps [0.05, 0.95] to [0, 1]
    const float MARGIN = 0.05f;  // 5% margin on each end
    rawDist = (rawDist - MARGIN) / (1.0f - 2*MARGIN);
    
    zombieDistances[i] = constrain(rawDist, 0.0, 1.0);
  }
  ...
  if (autoMode && systemEnabled && !gameOver) {
    ...
    // VERY CONSERVATIVE - only for TRUE emergencies (96%/94%)
    // Batch system handles normal sequencing - RARELY interrupt!
    //============================================
    int overrideLane = -1;
    float bestOverrideScore = 0;
    
    unsigned long timeSinceOverride = millis() - lastOverrideCommit;
    // MUCH longer cooldown between overrides - let batches COMPLETE
    // Only check every 150ms, and need 1200ms since last override
    bool canCheckOverride = (timeSinceOverride >= 1200) && (millis() - overrideCheckTime >= 150);
    ...
    if (canCheckOverride) {
      ...
      for (int i = 0; i < 4; i++) {
        if (i == committedLane) continue;
        if (ProxSensors[i].direction != FORWARD) continue;
        // MUCH longer anti-return period - don't switch back for 2 seconds
        if (i == lastOverrideLane && timeSinceOverride < 2000) continue;
        
        // VERY TIGHT OVERRIDE_THRESHOLD - only TRUE emergencies (96%/94%)
        if (zombieDistances[i] < OVERRIDE_THRESHOLD[i]) {
          float score = calculateThreatScore(i);
...
void chooseAndCommitTargetFromBatch() {
  ...
  if (nextLane >= 0) {
    // Print selection in analysis mode - show distances for each lane
    if (analysisMode) {
      Serial.print(F("[PRIORITY] "));
      // Show distances sorted by closest first
      int order[4] = {0, 1, 2, 3};
      // Sort by distance (closest first = highest %)
      for (int i = 0; i < 3; i++) {
        for (int j = i + 1; j < 4; j++) {
          if (zombieDistances[order[j]] < zombieDistances[order[i]]) {
            int temp = order[i];
            order[i] = order[j];
            order[j] = temp;
          }
      ...
      for (int i = 0; i < 4; i++) {
        int lane = order[i];
        int pct = (int)((1.0 - zombieDistances[lane]) * 100);
        Serial.print(F("L"));
        Serial.print(lane + 1);
        Serial.print(F(":"));
        Serial.print(pct);
...
void dwellAtTarget() {
  ...
  //--------------------------------------------
  if (currentDir == BACKWARD && dwellTime >= 150) {  // Increased from 50 - dwell longer before backward exit
  ...
  //--------------------------------------------
  if (dwellTime >= 200 && peakZombieDistance < 0.15) {  // Increased from 100 - dwell longer
    // Zombie got to within 15% (close to wall)
    float retreatAmount = currentDist - peakZombieDistance;
    if (retreatAmount > 0.06) {  // Increased from 4% to 6% - need more retreat to confirm
      zombiesKilled++;
      recordHit(activeTargetIndex);  // Analysis mode tracking
      Serial.print(F("HIT L"));
      Serial.print(activeTargetIndex + 1);
  ...
  //--------------------------------------------
  // EXIT CONDITION 2.5: ZOMBIE STOPPED - EXIT (but don't count as hit unless very close)
  // If zombie hasn't moved significantly, move on to other targets
  // Be CONSERVATIVE about counting stopped zombies as hits
  //--------------------------------------------
  // Check for STOPPED direction OR very small velocity (pseudo-stopped)
  bool isEffectivelyStopped = (currentDir == STOPPED) || 
                               (abs(zombieVelocities[activeTargetIndex]) < 0.00005);
  
  if (isEffectivelyStopped && dwellTime >= 300) {  // Increased from 150 - wait longer before STOP exit
    if (stoppedStartTime == 0) {
      stoppedStartTime = millis();
      stoppedStartDistance = currentDist;
    } else if (millis() - stoppedStartTime >= 500) {  // Increased from 300 - be more patient
      // Check if distance barely changed - zombie truly stalled
      float distChange = abs(currentDist - stoppedStartDistance);
      if (distChange < 0.05) {  // Tightened from 8% to 5% - must be truly stopped
        // ONLY count as hit if zombie got VERY close (85%+ = 15% distance or less)
        // This is more conservative - we don't want false hit credits
        if (peakZombieDistance < 0.15) {
          zombiesKilled++;
          ...
          // Zombie stopped but wasn't close enough - just move on, no hit credit
  ...
  } else if (currentDir != STOPPED && abs(zombieVelocities[activeTargetIndex]) > 0.0003) {
...
// CRITICAL: Different modes use different scoring strategies
// MODE_DISTANCE: Pure distance-based (closest zombie wins)
// MODE_TTI: Pure TTI-based (soonest impact wins)  
// MODE_HYBRID: Weighted combination of both
// BACKWARD = NO THREAT (period - no exceptions)
//
// EMERGENCY OVERRIDE: Any zombie at 80%+ (dist < 0.20) moving FORWARD
// gets MAXIMUM priority regardless of mode - prevents slip-throughs!
//============================================
float calculateThreatScore(int lane) {
  float dist = zombieDistances[lane];
  float score = 0;
  ...
  //============================================
  // EMERGENCY DISTANCE OVERRIDE - APPLIES TO ALL MODES!
  // Any zombie at 80%+ (dist < 0.20) moving forward is CRITICAL
  // This prevents zombies from slipping through while we're busy elsewhere
  //============================================
  if (dist < 0.08) {
    // CRITICAL EMERGENCY: 92%+ - about to impact!
    score = 2000 + (0.08 - dist) * 5000;  // 2000-2400 range
    score *= LANE_PRIORITY[lane];
    return score;  // Bypass all other calculations
  }
  if (dist < 0.15) {
    // SEVERE: 85%+ - must act NOW
    score = 1200 + (0.15 - dist) * 11400;  // 1200-2000 range  
    score *= LANE_PRIORITY[lane];
    return score;
  }
  if (dist < 0.20) {
    // URGENT: 80%+ - very high priority
    score = 800 + (0.20 - dist) * 8000;  // 800-1200 range
    score *= LANE_PRIORITY[lane];
    return score;
  }
  
  //============================================
  // MODE_DISTANCE: Pure distance-based scoring
  // Simply prioritize closest zombies (lowest distance = highest score)
  //============================================
  if (targetMode == MODE_DISTANCE) {
    // Score is inversely proportional to distance
    // Closer = higher score
    if (dist < 0.25) {
      score = 600 + (0.25 - dist) * 4000;  // 600-800 range (75%+)
    } else if (dist < 0.35) {
      score = 400 + (0.35 - dist) * 2000;  // 400-600 range (65-75%)
    } else if (dist < 0.50) {
      score = 200 + (0.50 - dist) * 1333;  // 200-400 range (50-65%)
    } else {
      score = 50 + (1.0 - dist) * 300;     // 50-200 range for far zombies
    }
    
    // Apply lane priority and travel time penalty
    score *= LANE_PRIORITY[lane];
    int dynamicTravel = getDynamicTravelTime(lane);
    score -= dynamicTravel * 0.1;  // Small travel penalty
    
    return max(score, 0.0f);
  }
  
  //============================================
  // MODE_TTI: Pure TTI-based scoring
  // Prioritize zombies that will hit soonest (lowest TTI = highest score)
  //============================================
  if (targetMode == MODE_TTI) {
    float effectiveTTI = getEffectiveTTI(lane);
    float rawTTI = timeToImpact[lane];
    
    if (rawTTI < 99999 && effectiveTTI < 5000) {
      // Valid TTI - use it directly
      if (effectiveTTI < 200) {
      ...
      // No valid TTI - use distance as fallback
      score = 50 + (1.0 - dist) * 100;
    }
  ...
  //============================================
  // MODE_HYBRID: Weighted combination of distance and TTI
  // This is the default mode - balances both factors
  //============================================
  float effectiveTTI = getEffectiveTTI(lane);
  float rawTTI = timeToImpact[lane];
  
  // TTI component (60% weight)
  float ttiScore = 0;
  if (rawTTI < 99999 && effectiveTTI < 5000) {
    if (effectiveTTI < 200) {
      ttiScore = 1000 + (200 - effectiveTTI) * 5;
    } else if (effectiveTTI < 500) {
      ttiScore = 600 + (500 - effectiveTTI) * 1.3;
    } else if (effectiveTTI < 1000) {
      ttiScore = 300 + (1000 - effectiveTTI) * 0.6;
    } else if (effectiveTTI < 2000) {
      ttiScore = 100 + (2000 - effectiveTTI) * 0.2;
    } else {
      ttiScore = 50 + (5000 - effectiveTTI) * 0.02;
    }
  }
  ...
  float distScore = 0;
  if (dist < 0.10) {
    distScore = 800 + (0.10 - dist) * 2000;
  } else if (dist < 0.20) {
    distScore = 400 + (0.20 - dist) * 4000;
  } else if (dist < 0.35) {
    distScore = 150 + (0.35 - dist) * 1700;
  } else {
    distScore = 30 + (1.0 - dist) * 80;
  }
  
  // Combine scores - TTI has more weight if available
  if (rawTTI < 99999 && effectiveTTI < 5000) {
    score = ttiScore * 0.6 + distScore * 0.4;
  } else {
    score = distScore;  // No TTI data, use distance only
  }
