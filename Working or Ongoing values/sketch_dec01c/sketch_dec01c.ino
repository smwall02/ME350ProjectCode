...
const unsigned long MIN_COMMITMENT_TIME = 1500;  // Minimum 1.5s commitment before allowing overrides (increased for better dwelling)
...
// calibrationMin = minimum sensor value seen = closest zombie (zombie at wall)
// calibrationMax = maximum sensor value seen = farthest zombie (no zombie)
// Lower sensor value = closer zombie
int calibrationMin[4] = {1023, 1023, 1023, 1023};  // Initialize to max (will track minimum)
int calibrationMax[4] = {0, 0, 0, 0};              // Initialize to min (will track maximum)
...
const unsigned long MIN_DWELL_TIME = 500;  // Increased from 250ms for better dwelling
const unsigned long NORMAL_DWELL_TIME = 800;  // Increased from 450ms
const unsigned long MAX_DWELL_TIME = 1200;  // Increased from 850ms
const unsigned long L4_DWELL_TIME = 1500;  // Increased from 1050ms
...
void calculateNewBatch() {
  ...
  // Start prioritizing coverage earlier - after 3 targets (halfway through sequence)
  bool needCoverage = (targetsInSequence >= 3);  // After 3 targets, start ensuring remaining lanes are covered
  int lanesNotHit = 0;
  for (int k = 0; k < 4; k++) {
  ...
  // COVERAGE CHECK: If we're at 3+ targets and some lanes haven't been hit, ensure they're in batch
...
void updateCalibration() {
  ...
  for (int i = 0; i < 4; i++) {
    ...
    // Track minimum and maximum sensor readings during calibration
    // Lower sensor value = closer zombie (zombie at wall)
    // Higher sensor value = farther zombie (no zombie)
    // calibrationMin = minimum value seen = closest zombie = zombie at wall
    // calibrationMax = maximum value seen = farthest zombie = no zombie
    
    if (rawVal < calibrationMin[i]) {
      calibrationMin[i] = rawVal;  // Track minimum (closest zombie)
      calibrationUpdated[i] = true;
    }
    if (rawVal > calibrationMax[i]) {
      calibrationMax[i] = rawVal;  // Track maximum (farthest/no zombie)
      calibrationUpdated[i] = true;
    }
  }
}
  ...
  for (int i = 0; i < 4; i++) {
    if (calibrationUpdated[i]) {
      ...
      if (range > 150) {
        ...
        // Add margins to handle sensor readings outside calibration range
        // Far: add margin so readings slightly above max still map correctly
        newFar = min(newFar + 20, 950);
        
        // Close: CRITICAL - ensure we capture the true minimum
        // If zombie didn't go all the way to wall during calibration, close value is too high
        // Subtract margin to account for readings below the calibrated minimum
        // This ensures sensor readings below the calibration min map to 0.0 (at impact)
        const int MIN_CLOSE_VALUE = 30;  // Absolute minimum reasonable sensor value
        const int MAX_CLOSE_VALUE = 150;  // Maximum reasonable close value
        
        // Close: subtract margin to handle sensor readings below calibration minimum
        // This is especially important for L4 which may not get fully calibrated
        if (i == 1 || i == 2) {
          // Short lanes (L2, L3) - zombie may not get as close, but still need margin
          newClose = max(newClose - 40, MIN_CLOSE_VALUE);
        } else {
          // Long lanes (L1, L4) - zombie should get closer, use larger margin
          // L4 especially needs this to handle readings below calibration
          newClose = max(newClose - 50, MIN_CLOSE_VALUE);
        }
        
        // CRITICAL: Cap close value to prevent bad calibration
        // But don't cap too aggressively - we want to capture true minimum
...
void loop() {
  ...
  if (autoMode && systemEnabled && !gameOver) {
    ...
    // After an override, wait at least 1 second before allowing another override (reduced for short lanes)
    const unsigned long OVERRIDE_COOLDOWN = 2000;  // 2 second cooldown after override (increased to prevent rapid switching)
    ...
    if (timeSinceOverride < OVERRIDE_COOLDOWN) {
      ...
      // While dwelling: Require minimum dwell time before allowing overrides
      unsigned long dwellStartTime = (commitStartTime > 0) ? commitStartTime : millis();
      unsigned long dwellDuration = millis() - dwellStartTime;
      // Require at least MIN_DWELL_TIME of dwelling before allowing overrides (except for extreme emergencies)
      if (dwellDuration < MIN_DWELL_TIME) {
        canCheckOverride = false;  // Haven't dwelled long enough yet
      } else {
        // After minimum dwell, check overrides every 500ms (slower to prevent rapid switching)
        canCheckOverride = (millis() - overrideCheckTime >= 500);
      }
    } else {
    ...
    if (canCheckOverride) {
      ...
      for (int i = 0; i < 4; i++) {
        ...
        if (batchLocked && batchActive) {
          ...
          // FIXED: Override MUCH earlier - at 50% through lane, not 25%!
          if (isShortLane && committedIsLongLane && dist < 0.50) {
            // Short lane at < 50% distance (50%+ through lane) while at long lane = override immediately!
            shouldOverride = true;
          }
          // PRIORITY 2: Short lane getting close while committed to another short lane
          ...
          // PRIORITY 3: Critical vs non-critical = override
          else if (isCritical && !committedIsCritical) {
            shouldOverride = true;
          }
          // PRIORITY 4: Critical and much closer (at least 30% closer)
          else if (isCritical && committedIsCritical && dist < committedDist * 0.7) {
            shouldOverride = true;
          }
          // PRIORITY 5: Short lane getting close while long lane is at less urgent distance
          ...
          // PRIORITY 6: Below override threshold and committed is not critical
    ...
    if (overrideLane >= 0 && shouldOverride(overrideLane)) {
      // CRITICAL: Double-check that target is still forward-moving before committing
      if (ProxSensors[overrideLane].direction != FORWARD) {
        // Target is not forward (backward or stopped) - skip this override
        overrideLane = -1;
      } else {
...
void chooseAndCommitTargetFromBatch() {
  ...
  if (batchLocked && batchActive) {
    ...
    if (nextLane >= 0) {
      ...
      // CRITICAL: Double-check that target is forward-moving before committing
      if (ProxSensors[nextLane].direction == BACKWARD) {
        // Target is backward - skip and get next
        nextLane = getNextBatchTarget();
        if (nextLane >= 0) {
          // Double-check next target too
          if (ProxSensors[nextLane].direction != BACKWARD) {
      ...
      if (isValidForShortLane || isValidStandard) {
        ...
        nextLane = getNextBatchTarget();
        if (nextLane >= 0) {
          // Double-check next target is not backward
          if (ProxSensors[nextLane].direction != BACKWARD) {
            commitToTarget(nextLane);
            if (isCommitted) {
              state = MOVE_TO_TARGET;
  ...
  if (nextLane >= 0) {
    ...
    if (ProxSensors[nextLane].direction != BACKWARD) {
      ...
      // Target is backward - skip and try to get another
      nextLane = getNextBatchTarget();
      if (nextLane >= 0 && ProxSensors[nextLane].direction != BACKWARD) {
        commitToTarget(nextLane);
        if (isCommitted) {
          state = MOVE_TO_TARGET;
        }
      } else {
        // No valid targets - go to wait position
        desiredPosition = WAIT_POSITION;
      }
    }
  } else {
...
void dwellAtTarget() {
  ...
  for (int i = 0; i < 4; i++) {
    ...
    if (shouldOverride) {
      float score = calculateThreatScore(i);
      if (isShortLane) {
        // FIXED: Boost scores for short lanes at earlier distances
        if (dist < 0.30) score += 20000;  // Huge boost at 30% through lane
        else if (dist < 0.50) score += 15000;  // Large boost at 50% through lane
        else if (dist < 0.70) score += 10000;  // Boost at 70% through lane
  ...
  if (overrideLane >= 0 && shouldOverride(overrideLane)) {
    ...
    if (ProxSensors[overrideLane].direction == FORWARD) {
      ...
      // Target became backward - clear override
      overrideLane = -1;
    }
  }
...
void updateSensors() {
  ...
  for (int i = 0; i < 4; i++) {
    ...
    if (range != 0.0f) {
      // Distance calculation: 0.0 = at impact (urgent!), 1.0 = far (at start)
      // ProxRange[i][0] = far value (high, no zombie)
      // ProxRange[i][1] = close value (low, zombie at wall)
      // Lower sensor value = closer zombie = lower distance value
      float normalized = (ProxSensors[i].currVal - ProxRange[i][1]) / range;
      // Handle cases where sensor reads below calibrated close value (zombie closer than calibration captured)
      if (normalized < 0.0f) {
        normalized = 0.0f;  // Clamp to 0.0 (at impact)
      }
      // Handle cases where sensor reads above calibrated far value
      if (normalized > 1.0f) {
        normalized = 1.0f;  // Clamp to 1.0 (far)
      }
      currentDistance = normalized;
